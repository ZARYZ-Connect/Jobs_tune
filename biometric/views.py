...
from JOBS_TUNE.JOBS_TUNE_settings import BIO_DEVICE_THREADS
from JOBS_TUNE.settings import TIME_ZONE
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_JOBS_TUNE_cosec_user(request, user_id, device_id):
    """
...
def map_biometric_users(request, device_id):
    """
    Maps an JOBS_TUNE employee to a biometric user on a specified biometric device.
    """
    device = BiometricDevices.find(device_id)
...
def dahua_biometric_attendance_logs(device):
    """
    Retrieves logs from a Dahua biometric device and marks attendance in JOBS_TUNE.

    This function fetches biometric logs from the specified device, processes the attendance records,
    and updates the attendance system in JOBS_TUNE. If an employee has an active clock-in record,
