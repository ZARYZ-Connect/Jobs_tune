...
from JOBS_TUNE.filters import JOBS_TUNEPaginator
from JOBS_TUNE.group_by import group_by_queryset
from JOBS_TUNE.JOBS_TUNE_settings import JOBS_TUNE_DATE_FORMATS
from JOBS_TUNE.methods import get_JOBS_TUNE_model_class
from JOBS_TUNE_audit.models import AccountBlockUnblock, HistoryTrackingFields
from JOBS_TUNE_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
@manager_can_enter("JOBS_TUNE_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="JOBS_TUNE_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("JOBS_TUNE_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "JOBS_TUNE_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "JOBS_TUNE_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("JOBS_TUNE_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("JOBS_TUNE_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("JOBS_TUNE_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("JOBS_TUNE_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("JOBS_TUNE_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = JOBS_TUNEPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_JOBS_TUNE_model_class(
                    ...
                    Stage = get_JOBS_TUNE_model_class(
                    ...
                    OnboardingStage = get_JOBS_TUNE_model_class(
                    ...
                    OnboardingTask = get_JOBS_TUNE_model_class(
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        JOBS_TUNE_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_JOBS_TUNE_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_JOBS_TUNE_model_class(
