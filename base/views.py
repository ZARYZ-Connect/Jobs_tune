...
from JOBS_TUNE import JOBS_TUNE_apps
from JOBS_TUNE.decorators import (
    delete_permission,
...
from JOBS_TUNE.group_by import group_by_queryset
from JOBS_TUNE.JOBS_TUNE_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from JOBS_TUNE.methods import get_JOBS_TUNE_model_class, remove_dynamic_url
from JOBS_TUNE_audit.forms import HistoryTrackingFieldsForm
from JOBS_TUNE_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/JOBS_TUNE_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/JOBS_TUNE_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/JOBS_TUNE_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/JOBS_TUNE_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/JOBS_TUNE_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/JOBS_TUNE_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/JOBS_TUNE_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/JOBS_TUNE_job_position.html",
...
class JOBS_TUNEPasswordResetView(PasswordResetView):
    """
    JOBS_TUNE View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    JOBS_TUNE View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not JOBS_TUNE_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_JOBS_TUNE_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(JOBS_TUNE_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/JOBS_TUNE-logo.png")
    company_name = "JOBS_TUNE"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from JOBS_TUNE")
...
@login_required
@hx_request_required
@permission_required("base.change_JOBS_TUNEmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = JOBS_TUNEMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_JOBS_TUNEmailtemplate")
...
@login_required
@permission_required("base.delete_JOBS_TUNEmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = JOBS_TUNEMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_JOBS_TUNE_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_JOBS_TUNE_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_JOBS_TUNE_model_class(
...
@login_required
@permission_required("JOBS_TUNE_audit.change_accountblockunblock")
...
@login_required
@permission_required("JOBS_TUNE_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("JOBS_TUNE_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("JOBS_TUNE_audit.change_audittag")
...
@login_required
def get_JOBS_TUNE_installed_apps(request):
