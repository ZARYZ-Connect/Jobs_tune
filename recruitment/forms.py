...
class RecruitmentCreationForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            self.fields["recruitment_managers"] = JOBS_TUNEMultiSelectField(
    ...
    def clean(self):
        if isinstance(self.fields["recruitment_managers"], JOBS_TUNEMultiSelectField):
...
class StageCreationForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        reload_queryset(self.fields)
        if not self.instance.pk:
            self.fields["stage_managers"] = JOBS_TUNEMultiSelectField(
                queryset=Employee.objects.filter(is_active=True),
                widget=JOBS_TUNEMultiSelectWidget(
                    filter_route_name="employee-widget-filter",
                    filter_class=EmployeeFilter,
    ...
    def clean(self):
        if isinstance(self.fields["stage_managers"], JOBS_TUNEMultiSelectField):
