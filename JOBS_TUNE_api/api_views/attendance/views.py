        ...
        if not request.user.employee_get.check_online():
            ...
            if employee and work_info is not None:
                ...
                if start_time_sec > end_time_sec:
                    # night shift
                    # ------------------
                    # Night shift in JOBS_TUNE consider a 24 hours from noon to next day noon,
...
class MailTemplateView(APIView):
    ...
    def get(self, request):
        instances = JOBS_TUNEMailTemplate.objects.all()
...
class ConvertedMailTemplateConvert(APIView):
    ...
    def put(self, request):
        ...
        bdy = JOBS_TUNEMailTemplate.objects.filter(id=template_id).first()
